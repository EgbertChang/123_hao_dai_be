...
const selectAllASql = "SELECT id, name, (SELECT COUNT(*) FROM partyB WHERE partyAId=partyA.id) AS partyBNum FROM partyA WHERE name LIKE ? LIMIT ?, ?"

const deleteASql = "DELETE FROM partyA WHERE id = ?"

const selectAInfoSql = "SELECT id, name, (SELECT COUNT(*) FROM partyB WHERE partyAId=partyA.id) AS partyBNum FROM partyA WHERE id = ?"
...
const selectProductListSql = `SELECT id, name, limitMin, limitMax, slogan, applyNumber, interest FROM product WHERE name LIKE ? LIMIT ?, ?`
